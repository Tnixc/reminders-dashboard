...
func (k KeyMap) FullHelp() [][]key.Binding {
	...
	case config.RemindersView:
		...
		customKeys = append(customKeys, CustomRemindersBindings...)
	default:
		additionalKeys = []key.Binding{}
		customKeys = append(customKeys, []key.Binding{}...)
	}
...
func Rebind(universal, remindersKeys []config.Keybinding) error {
	err := rebindUniversal(universal)
	if err != nil {
		return err
	}

	return rebindRemindersKeys(remindersKeys)
}
...
var (
	...
	CustomRemindersBindings []key.Binding
)
...
func rebindRemindersKeys(keys []config.Keybinding) error {
	CustomRemindersBindings = []key.Binding{}

	for _, reminderKey := range keys {
		if reminderKey.Builtin == "" {
			// Handle custom commands
			if reminderKey.Command != "" {
				name := reminderKey.Name
				if reminderKey.Name == "" {
					name = config.TruncateCommand(reminderKey.Command)
				}

				customBinding := key.NewBinding(
					key.WithKeys(reminderKey.Key),
					key.WithHelp(reminderKey.Key, name),
				)

				CustomRemindersBindings = append(CustomRemindersBindings, customBinding)
			}
			continue
		}

		// No built-in reminder keys for now
		return fmt.Errorf("unknown built-in reminder key: '%s'", reminderKey.Builtin)
	}
